...
//	ArrayList<Movie> tmpMovie = new ArrayList<Movie>();
	...
	void inputMovie() throws ChoiceException { // ?ûÖ?†•
	...
	void updateMovie() throws ChoiceException, NotExistException { // ?àò?†ï
	...
	void deleteMovie() throws NotExistException { // ?Ç≠?†ú
	...
	void deleteSerialNumber(int mvSerialNumber) { // ?ãúÎ¶¨Ïñº ?ÑòÎ≤? Í≥µÎ∞± ?†úÍ±?
		Iterator<Movie> it = movieStorage.iterator();
		while (it.hasNext()) {
			Movie tmp = it.next();
	...
	public Movie search(String title) { // ?èô?ùº ?†úÎ™? Í≤??Éâ
		Iterator<Movie> it = movieStorage.iterator();
		while (it.hasNext()) {
			Movie tmp = it.next();
	...
	void counting(String title) { // ?èô?ùº ?†úÎ™? Í∞??àò Ïπ¥Ïö¥?åÖ
		if (!movieStorageCheck()) {
			return;
		} // end if
		Iterator<Movie> it = movieStorage.iterator();
	...
	private Movie serialNumberSearch(int tmpNumber) { // ?èô?ùº ?ãúÎ¶¨Ïñº?ÑòÎ≤? Í≤??Éâ
		Iterator<Movie> it = movieStorage.iterator();
		Movie tmp = new Movie();
		while (it.hasNext()) {
			tmp = it.next();
